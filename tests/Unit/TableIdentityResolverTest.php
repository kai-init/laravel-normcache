...
final class TableIdentityResolverTest extends UnitTestCase
{
    ...
    public function test_postgres_takes_the_first_search_path_entry_without_asking_the_database(): void
    {
        $connection = $this->connection('pgsql', ['search_path' => 'reporting, public']);
        $connection->shouldNotReceive('getSchemaBuilder');

        $identity = app(TableIdentityResolver::class)->resolve($connection, 'posts');

        $this->assertSame('reporting', $identity?->schema);
    }

    public function test_a_user_search_path_resolves_to_the_session_user_per_tenant(): void
    {
        $resolver = app(TableIdentityResolver::class);
        $path = ['username' => null, 'search_path' => '"$user", public'];

        $a = $resolver->resolve($this->connection('pgsql', [...$path, 'username' => 'tenant_a']), 'posts');
        $b = $resolver->resolve($this->connection('pgsql', [...$path, 'username' => 'tenant_b']), 'posts');

        $this->assertSame('tenant_a', $a?->schema);
        $this->assertSame('tenant_b', $b?->schema);
        $this->assertNotSame(
            $a?->hash,
            $b?->hash,
            'two tenants differing only by username must not share a table identity',
        );
    }

    public function test_an_empty_user_falls_through_to_the_next_search_path_entry(): void
    {
        $identity = app(TableIdentityResolver::class)->resolve(
            $this->connection('pgsql', ['username' => '', 'search_path' => '"$user", public']),
            'posts',
        );

        $this->assertSame('public', $identity?->schema);
    }
