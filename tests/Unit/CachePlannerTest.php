...
class CachePlannerTest extends UnitTestCase
{
    public function test_successful_hot_plan_does_not_build_reason_strings(): void
    {
        ...
        $plan = $this->planner()->plan($prepared->builder, $prepared->base, CachePlanContext::models());

        $this->assertTrue($plan->usesModelCache());
    ...
    public function test_removed_soft_delete_scope_does_not_ignore_a_manual_null_constraint(): void
    {
        ...
        $this->assertSame(CacheStrategy::ModelIndex, $plan->strategy);
    }
    ...
    public function test_simple_scalar_uses_result_strategy(): void
    {
        $prepared = Author::where('name', 'Alice')->prepareCacheExecution();
        $plan = $this->planner()->plan($prepared->builder, $prepared->base, CachePlanContext::scalar(['*']));
    ...
    public function test_grouped_scalar_preserves_result_cache_behavior(): void
    {
        ...
        $plan = $this->planner()->plan($prepared->builder, $prepared->base, CachePlanContext::scalar(['name']));
    ...
    {
        return $this->app->make(CachePlanner::class);
    }
}
