...
final class QueryPlannerTest extends UnitTestCase
{
    ...
    public function test_root_wildcard_uses_canonical_rows_with_automatic_overlay_admission(): void
    {
        ...
        $this->assertSame([], $plan->predicateColumns);
    }

    public function test_canonical_plan_carries_predicate_and_order_columns(): void
    {
        $query = RawPost::query()->toBase()->from('posts')
        ...
        $this->assertSame(QueryPlan::CANONICAL, $plan->route);
        $this->assertSame(['published', 'title'], $plan->predicateColumns);
    }

    public function test_canonical_plan_with_an_unparseable_predicate_carries_no_predicate_columns(): void
    {
        $query = RawPost::query()->toBase()->from('posts')->whereRaw('published = ?', [true]);
        ...
        $this->assertSame(QueryPlan::CANONICAL, $plan->route);
        $this->assertNull($plan->predicateColumns);
    }
