...
final class CacheKeyBuilderTest extends UnitTestCase
{
    ...
    {
        $table = TableIdentity::fromParts('mysql', 'prod', 'main', 'app', 'app', '', 'posts');
        ...
        $this->assertStringContainsString($expectedTag, $keys->exact($table, '12', 'u', 'abc'));
        $this->assertStringContainsString($expectedTag, $keys->row($table, '4', 'i:42'));
        $this->assertStringContainsString($expectedTag, $keys->guard($table, 'i:42'));
    }
