...
final class WriteInvalidationTest extends TestCase
{
    ...
    public function test_primary_key_mutation_invalidates_the_old_and_new_tokens(): void
    {
        DB::table('posts')->orderBy('id')->get();
        $identity = app(TableIdentityResolver::class)
            ->resolve(DB::connection(), 'posts');
        ...
        $this->assertSame('1', $this->cacheStore()->getRaw(
            $keys->guard($identity, 'i:' . $this->postId),
        ));
        $this->assertSame('1', $this->cacheStore()->getRaw(
            $keys->guard($identity, 'i:' . $newId),
        ));
        $this->assertSame(
            $generation,
            $this->cacheStore()->getRaw($keys->generation($identity)) ?? '0',
