...
final class CanonicalReadTest extends TestCase
{
    ...
    public function test_canonical_membership_tokens_must_match_the_primary_key_family(): void
    {
        ...
        // Preserve the hash so token-family validation runs.
        $this->cacheStore()->writeHashField(
            $membershipKey,
            'm',
            $this->app->make(MembershipCodec::class)->encode(
                epoch: $membership->epoch,
                generation: $membership->generation,
                rootVersion: (string) $membership->rootVersion,
                ids: ['s:' . $this->postId],
    ...
    public function test_foreign_qualified_deleted_at_predicate_is_not_misclassified_as_a_direct_lookup(): void
    {
        ...
        // Warm direct-row state without compiling the invalid query.
