...
final class CommitFailingPdo extends \PDO
{
    ...
    {
        throw new \PDOException('SQLSTATE[HY000]: server has gone away');
    }
}
...
final class WriteInvalidationTest extends TestCase
{
    ...
    public function test_raw_predicate_widening_a_write_evicts_the_rows_it_reached(): void
    {
        $otherId = (int) RawPost::query()->toBase()->insertGetId([
            'title' => 'Before',
            'views' => 0,
            'published' => true,
        ...
        // Exercise generation-scoped rows that precise invalidation evicts by token.
        $read = static fn(int $id): string => RawPost::query()
            ->toBase()
            ->where('id', $id)
            ->first()
            ->title;

        $this->assertSame('Before', $read($this->postId));
        $this->assertSame('Before', $read($otherId));

        // The ungrouped OR reaches both rows.
        $affected = RawPost::query()->toBase()
            ->where('id', $this->postId)
            ->whereRaw("1 = 1 or id = {$otherId}")
            ->update(['title' => 'After']);

        $this->assertSame(2, $affected);
        $this->assertSame('After', $read($this->postId));
        $this->assertSame('After', $read($otherId));
    }
    ...
    public function test_a_commit_that_throws_has_its_queue_drained_by_the_next_transaction(): void
    {
        ...
        DB::connection()->setPdo(new CommitFailingPdo('sqlite:' . $database));

        DB::beginTransaction();
        RawPost::query()->toBase()->where('id', $this->postId)->update(['title' => 'Ambiguous']);

        try {
            DB::commit();
            $this->fail('The simulated commit failure was not thrown.');
        } catch (\PDOException) {
            // Laravel emits no transaction completion event here.
        }

        $this->assertSame($before, $this->tableVersion(), 'nothing can drain it yet');

        DB::connection()->setPdo(new \PDO('sqlite:' . $database));
        DB::beginTransaction();
        DB::rollBack();
        ...
        $this->assertNotSame(
            $before,
            $this->tableVersion(),
            'the abandoned queue must be drained when the next transaction opens',
        );

        // The failed commit left the row unchanged; the query log proves cache eviction.
        DB::flushQueryLog();
        DB::enableQueryLog();
        $this->assertSame('Before', $read());
        DB::disableQueryLog();
    ...
    public function test_an_inner_rollback_does_not_discard_the_outer_transactions_invalidation(): void
    {
        $read = fn(): ?string => RawPost::query()->toBase()->where('id', $this->postId)->value('title');
        ...
        DB::beginTransaction();
        RawPost::query()->toBase()->where('id', $this->postId)->update(['title' => 'Outer']);
        DB::beginTransaction();
        DB::rollBack();
        DB::commit();

        $this->assertNotSame($before, $this->tableVersion());
        $this->assertSame('Outer', $read());
    }
    ...
    public function test_update_or_insert_invalidates_exactly_when_its_nested_update_reports_rows(): void
    {
        ...
        // Drivers disagree whether value-preserving updates affect a row.
