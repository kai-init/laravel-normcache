...
final class PublicInvalidationTest extends TestCase
{
    ...
    {
        $read = fn() => RawPost::query()->toBase()->where('id', $this->postId)->first();
        $read();
        $read();
    ...
    {
        $tagged = fn() => RawPost::query()->toBase()->where('id', $this->postId)->tag('homepage')->get();
        $untagged = fn() => RawPost::query()->toBase()->where('id', $this->postId)->get();
    ...
    public function test_model_invalidation_applies_the_connection_before_resolving_the_table(): void
    {
        ...
        try {
            ...
            $readPrimary = static fn() => PublicConnectionAwareInvalidationModel::on($connectionName)
                ->toBase()
                ->where('id', 1)
                ->value('name');
