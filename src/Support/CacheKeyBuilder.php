...
final class CacheKeyBuilder
{
    ...
    public function changeRecord(TableIdentity $table, string $version): string
    {
        return $this->changeRecordPrefix($table) . $version;
    }

    public function changeRecordPrefix(TableIdentity $table): string
    {
        return $this->tablePrefix($table) . ':chg:';
    }
    ...
    public function queryEntry(
        ...
        return $this->tablePrefix($table) . ":q:{$namespace}:{$queryHash}";
    }
