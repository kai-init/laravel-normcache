...
{
    private const EPOCH_REFRESH_SECONDS = 5;
    ...
    {
        return $this->epochReadAt !== null
            && (microtime(true) - $this->epochReadAt) >= self::EPOCH_REFRESH_SECONDS;
    }
    ...
    private function resolveState(): array
    {
        if ($this->epoch === null || $this->epochExpired()) {
            ...
            $this->epochReadAt = microtime(true);
            // Refreshes expose remote kill switches to an active scope.
